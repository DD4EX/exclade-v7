...
import { nonTechnicalCoordinators } from "@/data/crew";
...
export function ChaosZone() {
  ...
  const acceptOperation = (event: NonTechnicalEvent) => {
      ...
      <div className="chaos-inner">
        ...
        <div className="chaos-card-grid">
          {nonTechnicalEvents.map((event, index) => {
            ...
            const coordinator = nonTechnicalCoordinators.find((person) => person.assignment.toUpperCase() === event.name.toUpperCase())?.name ?? "EXCLADE OPERATIONS";
            return (
              <article
              ...
              >
                ...
                {catalog && <dl className="chaos-card-meta"><div><dt>TIME</dt><dd>{catalog.time}</dd></div><div><dt>VENUE</dt><dd>{catalog.venue}</dd></div><div><dt>COORDINATOR</dt><dd>{coordinator}</dd></div></dl>}
                <p className="chaos-card-desc">{summary}</p>
                <button
                  type="button"
                  className="chaos-accept"
