import { Link } from "@tanstack/react-router";
import { useEffect, useMemo, useState } from "react";
import { CheckCircle2, Lock, MailCheck, Terminal, Upload } from "lucide-react";

import { PaymentQrPanel } from "@/components/PaymentQrPanel";
import {
  catalogEvents,
  daysFor,
  eventsByDay,
  feeForEvent,
  findEvent,
  maxTeamSizeFor,
  type EventDay,
} from "@/data/eventCatalog";
import { buildPaymentNote, generatePaymentQrDataUrl, type PaymentQr } from "@/lib/paymentQr";
import { generateRegistrationId, submitRegistration } from "@/lib/registration";
import { supabase } from "@/integrations/supabase/client";

type Fields = {
  fullName: string;
  college: string;
  department: string;
  year: string;
  teamLeaderGmail: string;
  phone: string;
};

const emptyFields: Fields = {
  fullName: "",
  college: "",
  department: "",
  year: "",
  teamLeaderGmail: "",
  phone: "",
};

const labels: Record<keyof Fields, string> = {
  fullName: "FULL NAME / TEAM LEADER NAME",
  college: "COLLEGE / INSTITUTION",
  department: "DEPARTMENT",
  year: "YEAR",
  teamLeaderGmail: "TEAM LEADER GMAIL",
  phone: "PHONE NUMBER",
};

const years = ["I", "II", "III", "IV"];

type ErrorKey = keyof Fields | "events" | "teamName" | "screenshot";
type Step = 1 | 2 | 3 | 4;

type PaymentState = {
  registrationId: string;
  eventId: string;
  eventName: string;
  eventDay: EventDay;
  registrationAmount: number;
  registrationType: "solo" | "team";
  teamName: string;
  teamLeaderName: string;
  teamLeaderGmail: string;
  assignedUpiAccount: string;
  assignedUpiName: string;
  assignedUpiId: string;
  dailyTransactionSlot: number;
  paymentNote: string;
  qrDataUrl: string;
};

const steps: { id: Step; label: string }[] = [
  { id: 1, label: "CHOOSE EVENT" },
  { id: 2, label: "TEAM DETAILS" },
  { id: 3, label: "PAYMENT" },
  { id: 4, label: "DONE" },
];

export function RegistrationSection() {
  const [step, setStep] = useState<Step>(1);
  const [fields, setFields] = useState<Fields>(emptyFields);
  const [selected, setSelected] = useState<string[]>([]);
  const [teamName, setTeamName] = useState("");
  const [members, setMembers] = useState<string[]>([]);
  const [screenshot, setScreenshot] = useState<File | null>(null);
  const [errors, setErrors] = useState<Partial<Record<ErrorKey, string>>>({});
  const [busy, setBusy] = useState(false);
  const [formError, setFormError] = useState("");
  const [paymentState, setPaymentState] = useState<PaymentState | null>(null);

  useEffect(() => {
    const eventId = new URLSearchParams(window.location.search).get("event");
    const event = eventId ? findEvent(eventId) : undefined;
    if (!event) return;
    setSelected([event.id]);
    setMembers([]);
    setErrors({});
    setStep(2);
  }, []);

  const days = useMemo(() => daysFor(selected), [selected]);
  const maxTeam = useMemo(() => maxTeamSizeFor(selected), [selected]);
  const selectedEvents = useMemo(
    () => selected.map((id) => findEvent(id)).filter((e): e is NonNullable<typeof e> => Boolean(e)),
    [selected],
  );
  const selectedEvent = selectedEvents[0] ?? null;
  const primaryDay: EventDay = days[0] ?? 1;

  const clearError = (key: ErrorKey) =>
    setErrors((prev) => {
      const next = { ...prev };
      delete next[key];
      return next;
    });

  const setField = (key: keyof Fields, value: string) => {
    setFields((prev) => ({ ...prev, [key]: value }));
    clearError(key);
  };

  // One event per registration: selecting an event replaces the previous choice.
  const toggleEvent = (id: string) => {
    setSelected((prev) => (prev[0] === id ? prev : [id]));
    setPaymentState(null);
    clearError("events");
  };

  const setMember = (index: number, value: string) => {
    setMembers((prev) => {
      const next = [...prev];
      next[index] = value;
      return next;
    });
  };

  const goToDetails = () => {
    if (selected.length === 0) {
      setErrors({ events: "SELECT ONE EVENT" });
      return;
    }
    setMembers((prev) => prev.slice(0, Math.max(0, maxTeamSizeFor(selected) - 1)));
    setStep(2);
  };

  const goToPayment = async () => {
    const next: Partial<Record<ErrorKey, string>> = {};
    (Object.keys(labels) as (keyof Fields)[]).forEach((key) => {
      if (!fields[key].trim()) next[key] = "FIELD REQUIRED";
    });
    if (fields.teamLeaderGmail.trim() && !/^[^\s@]+@(?:gmail\.com|googlemail\.com)$/i.test(fields.teamLeaderGmail.trim())) {
      next.teamLeaderGmail = "ENTER A VALID GMAIL ADDRESS";
    }
    if (fields.phone.trim() && !/^[+]?\d[\d\s-]{7,14}$/.test(fields.phone.trim())) {
      next.phone = "ENTER A VALID PHONE NUMBER";
    }
    if (maxTeam > 1 && (!teamName.trim() || teamName.trim().length < 2)) {
      next.teamName = "TEAM NAME REQUIRED";
    }

    setErrors(next);
    if (Object.keys(next).length > 0) return;
    if (!selectedEvent) {
      setErrors({ events: "SELECT ONE EVENT" });
      return;
    }

    const registrationId = generateRegistrationId();

    try {
      const { data, error } = await supabase.rpc("claim_upi_slot", {
        p_day: primaryDay,
        p_event_name: selectedEvent.name,
        p_registration_id: registrationId,
      });

      const result = Array.isArray(data) ? data[0] : data;
      if (error || !result) {
        const isMissingFunction = error?.code === "404" || error?.message?.toLowerCase().includes("does not exist") || error?.message?.toLowerCase().includes("not found");

        setFormError(
          isMissingFunction
            ? "PAYMENT SLOT SERVICE UNAVAILABLE. RUN THE SUPABASE MIGRATION FOR claim_upi_slot."
            : "DAY PAYMENT CAPACITY REACHED",
        );
        setStep(2);
        return;
      }

      const assigned = result as {
        registration_id: string;
        assigned_day: EventDay;
        assigned_upi_account: string;
        assigned_upi_name: string;
        assigned_upi_id: string;
        daily_transaction_slot: number;
      };

      const paymentNote = buildPaymentNote({
        registrationId: assigned.registration_id,
        eventName: selectedEvent.name,
        teamName: teamName.trim(),
        teamLeaderName: fields.fullName.trim(),
      });

      const qrDataUrl = await generatePaymentQrDataUrl({
        upiId: assigned.assigned_upi_id,
        payeeName: assigned.assigned_upi_name,
        amount: selectedEvent.fee,
        note: paymentNote,
      });

      setPaymentState({
        registrationId: assigned.registration_id,
        eventId: selectedEvent.id,
        eventName: selectedEvent.name,
        eventDay: assigned.assigned_day,
        registrationAmount: selectedEvent.fee,
        registrationType: maxTeam > 1 ? "team" : "solo",
        teamName: teamName.trim(),
        teamLeaderName: fields.fullName.trim(),
        teamLeaderGmail: fields.teamLeaderGmail.trim(),
        assignedUpiAccount: assigned.assigned_upi_account,
        assignedUpiName: assigned.assigned_upi_name,
        assignedUpiId: assigned.assigned_upi_id,
        dailyTransactionSlot: assigned.daily_transaction_slot,
        paymentNote,
        qrDataUrl,
      });

      setFormError("");
      setStep(3);
    } catch {
      setFormError("DAY PAYMENT CAPACITY REACHED");
      setStep(2);
    }
  };

  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError("");

    if (!paymentState) {
      setFormError("PAYMENT CHANNEL UNAVAILABLE");
      return;
    }
    if (!screenshot) {
      setErrors({ screenshot: "UPLOAD YOUR PAYMENT SCREENSHOT" });
      return;
    }

    setBusy(true);

    const result = await submitRegistration({
      registrationId: paymentState.registrationId,
      eventId: paymentState.eventId,
      eventName: paymentState.eventName,
      eventDay: paymentState.eventDay,
      registrationAmount: paymentState.registrationAmount,
      registrationType: paymentState.registrationType,
      teamName: paymentState.teamName,
      teamLeaderName: paymentState.teamLeaderName,
      teamLeaderGmail: paymentState.teamLeaderGmail,
      college: fields.college,
      department: fields.department,
      year: fields.year,
      email: fields.teamLeaderGmail.trim(),
      phone: fields.phone,
      teamMembers: members.filter((m) => m.trim()).map((name) => ({ name: name.trim() })),
      assignedUpiAccount: paymentState.assignedUpiAccount,
      assignedUpiName: paymentState.assignedUpiName,
      assignedUpiId: paymentState.assignedUpiId,
      dailyTransactionSlot: paymentState.dailyTransactionSlot,
      paymentNote: paymentState.paymentNote,
      qrDataUrl: paymentState.qrDataUrl,
      screenshot,
    });

    setBusy(false);

    if (!result.ok) {
      setFormError(result.message);
      return;
    }

    setStep(4);
  };

  const reset = () => {
    setFields(emptyFields);
    setSelected([]);
    setTeamName("");
    setMembers([]);
    setScreenshot(null);
    setErrors({});
    setFormError("");
    setPaymentState(null);
    setStep(1);
  };

  return (
    <section id="register" className="register-section" aria-labelledby="register-title">
      <div className="register-grid-bg" aria-hidden="true" />
      <div className="register-inner">
        <div className="lab-heading reveal-on-scroll">
          <div>
            <p className="eyebrow">REGISTRATION</p>
            <h2 id="register-title">REGISTRATION TERMINAL</h2>
            <p className="register-subtitle">CHOOSE EVENT → TEAM DETAILS → PAY → UPLOAD PROOF</p>
          </div>
          <span className="file-count"><Lock aria-hidden="true" size={12} /> EXCLADE 2K26 // SECURE ACCESS</span>
        </div>

        <div className="terminal-shell reveal-on-scroll">
          <div className="terminal-bar">
            <span><Terminal aria-hidden="true" size={13} /> REGISTRATION TERMINAL</span>
            <span>{step === 4 ? "SESSION COMPLETE" : `STEP ${step} / 3`}</span>
          </div>

          <ol className="register-steps" aria-label="Registration progress">
            {steps.map((s) => (
              <li key={s.id} className={s.id === step ? "is-current" : s.id < step ? "is-done" : ""}>
                <span aria-hidden="true">{s.id < step ? "✓" : s.id}</span>
                {s.label}
              </li>
            ))}
          </ol>

          {step === 1 && (
            <div className="register-form">
              <fieldset className="register-fieldset">
                <legend>STEP 1 — SELECT YOUR EVENT</legend>
                <p className="register-hint">
                  Pick one event. Team size and the payment channel are decided by your choice.
                </p>

                {([1, 2] as EventDay[]).map((day) => (
                  <div key={day}>
                    <p className={`register-group-label${day === 2 ? " register-group-label-chaos" : ""}`}>
                      DAY {day}
                    </p>
                    <div className="event-selector">
                      {eventsByDay(day).map((event) => (
                        <label
                          className={`event-option${event.category === "NON-TECHNICAL" ? " event-option-chaos" : ""}${selected.includes(event.id) ? " event-option-active" : ""}`}
                          key={event.id}
                        >
                          <input
                            type="radio"
                            name="exclade-event"
                            checked={selected[0] === event.id}
                            onChange={() => toggleEvent(event.id)}
                          />
                          <span className="event-option-box" aria-hidden="true" />
                          <span className="event-option-name">
                            {event.name}
                            <small>
                              {event.venue} · {event.time} ·{" "}
                              {event.maxTeam === 1 ? "SOLO" : `TEAM OF ${event.minTeam}–${event.maxTeam}`}
                            </small>
                          </span>
                        </label>
                      ))}
                    </div>
                  </div>
                ))}

                {errors.events && <span className="register-error" role="alert">{errors.events}</span>}
              </fieldset>

              <div className="register-submit-row">
                <p className="register-demo-note">
                  {selected.length > 0
                    ? `${selectedEvents[0]?.name ?? ""} · DAY ${days.join(" & ")}`
                    : "No event selected yet."}
                </p>
                <button type="button" className="primary-cta" onClick={goToDetails}>
                  CONTINUE <span aria-hidden="true">→</span>
                </button>
              </div>
            </div>
          )}

          {step === 2 && (
            <div className="register-form">
              <fieldset className="register-fieldset">
                <legend>STEP 2 — PARTICIPANT DETAILS</legend>
                <div className="register-fields">
                  {(Object.keys(labels) as (keyof Fields)[]).map((key) => (
                    <div
                      className={`register-field${errors[key] ? " register-field-error" : ""}${fields[key].trim() && !errors[key] ? " register-field-ok" : ""}`}
                      key={key}
                    >
                      <label htmlFor={`reg-${key}`}>{labels[key]}</label>
                      {key === "year" ? (
                        <select
                          id="reg-year"
                          value={fields.year}
                          onChange={(e) => setField("year", e.target.value)}
                          aria-invalid={Boolean(errors.year)}
                        >
                          <option value="">SELECT YEAR</option>
                          {years.map((y) => <option key={y} value={y}>{y} YEAR</option>)}
                        </select>
                      ) : (
                        <input
                          id={`reg-${key}`}
                          type={key === "teamLeaderGmail" ? "email" : key === "phone" ? "tel" : "text"}
                          value={fields[key]}
                          onChange={(e) => setField(key, e.target.value)}
                          aria-invalid={Boolean(errors[key])}
                          autoComplete={key === "fullName" ? "name" : key === "teamLeaderGmail" ? "email" : key === "phone" ? "tel" : "off"}
                        />
                      )}
                      {errors[key] && <span className="register-error">{errors[key]}</span>}
                    </div>
                  ))}
                </div>
              </fieldset>

              {maxTeam > 1 ? (
                <fieldset className="register-fieldset">
                  <legend>TEAM DETAILS</legend>
                  <p className="register-hint">
                    Your selection allows a team of up to {maxTeam}. You are member 1 — add your teammates below.
                  </p>
                  <div className="register-fields">
                    <div className={`register-field${errors.teamName ? " register-field-error" : ""}`}>
                      <label htmlFor="reg-team-name">TEAM NAME</label>
                      <input
                        id="reg-team-name"
                        value={teamName}
                        onChange={(e) => {
                          setTeamName(e.target.value);
                          clearError("teamName");
                        }}
                      />
                      {errors.teamName && <span className="register-error">{errors.teamName}</span>}
                    </div>
                    {Array.from({ length: maxTeam - 1 }).map((_, i) => (
                      <div className="register-field" key={i}>
                        <label htmlFor={`reg-member-${i}`}>MEMBER {i + 2} NAME (OPTIONAL)</label>
                        <input
                          id={`reg-member-${i}`}
                          value={members[i] ?? ""}
                          onChange={(e) => setMember(i, e.target.value)}
                        />
                      </div>
                    ))}
                  </div>
                </fieldset>
              ) : (
                <fieldset className="register-fieldset">
                  <legend>TEAM DETAILS</legend>
                  <p className="register-hint">This is a solo registration. No teammates are required.</p>
                </fieldset>
              )}

              {formError && <p className="register-error" role="alert">{formError}</p>}

              <div className="register-submit-row">
                <button type="button" className="secondary-cta" onClick={() => setStep(1)}>[ BACK ]</button>
                <button type="button" className="primary-cta" onClick={() => void goToPayment()}>
                  CONTINUE TO PAYMENT <span aria-hidden="true">→</span>
                </button>
              </div>
            </div>
          )}

          {step === 3 && (
            <form className="register-form" onSubmit={(e) => void onSubmit(e)} noValidate>
              <fieldset className="register-fieldset">
                <legend>STEP 3 — PAY THE REGISTRATION FEE</legend>
                <p className="register-hint">
                  Complete the payment with the assigned UPI account below, then upload the successful payment screenshot.
                </p>

                {paymentState ? (
                  <div className="payment-qr-row">
                    <PaymentQrPanel
                      qr={{
                        id: paymentState.assignedUpiAccount,
                        holder: paymentState.assignedUpiName,
                        upiId: paymentState.assignedUpiId,
                        displayName: paymentState.assignedUpiName,
                      }}
                      day={paymentState.eventDay}
                      amount={paymentState.registrationAmount}
                      note={paymentState.paymentNote}
                      qrDataUrl={paymentState.qrDataUrl}
                    />
                  </div>
                ) : (
                  <p className="register-error">{formError || "DAY PAYMENT CAPACITY REACHED"}</p>
                )}
              </fieldset>

              <fieldset className="register-fieldset">
                <legend>UPLOAD PAYMENT SCREENSHOT</legend>
                <div className={`register-field${errors.screenshot ? " register-field-error" : ""}`}>
                  <label htmlFor="reg-screenshot">
                    <Upload aria-hidden="true" size={12} /> PAYMENT SCREENSHOT (JPG, JPEG, PNG, MAX 5 MB)
                  </label>
                  <input
                    id="reg-screenshot"
                    type="file"
                    accept="image/jpeg,image/png,image/jpg"
                    onChange={(e) => {
                      setScreenshot(e.target.files?.[0] ?? null);
                      clearError("screenshot");
                    }}
                  />
                  {screenshot && <span className="register-hint">SELECTED: {screenshot.name}</span>}
                  {errors.screenshot && <span className="register-error">{errors.screenshot}</span>}
                </div>
              </fieldset>

              {formError && <p className="register-error" role="alert">{formError}</p>}

              <div className="register-submit-row">
                <button type="button" className="secondary-cta" onClick={() => setStep(2)}>[ BACK ]</button>
                <button type="submit" className="primary-cta" disabled={busy || !paymentState}>
                  {busy ? "SUBMITTING…" : "SUBMIT REGISTRATION"} <span aria-hidden="true">→</span>
                </button>
              </div>
            </form>
          )}

          {step === 4 && (
            <div className="register-success" role="status" aria-live="polite">
              <span className="register-success-mark" aria-hidden="true"><CheckCircle2 size={30} strokeWidth={1.4} /></span>
              <p className="register-success-kicker">REGISTRATION RECEIVED</p>
              <h3>EXCLADE 2K26</h3>
              <p className="register-success-welcome">REGISTRATION ID<br /><b>{paymentState?.registrationId ?? "EX26-XXXX"}</b></p>
              <dl className="file-facts">
                <div><dt>EVENT</dt><dd>{selectedEvents.map((e) => e.name).join(" · ")}</dd></div>
                <div><dt>DAY</dt><dd>{days.join(" & ")}</dd></div>
                <div><dt>TEAM LEADER</dt><dd>{fields.fullName}</dd></div>
                <div><dt>TEAM</dt><dd>{teamName || "INDIVIDUAL"}</dd></div>
              </dl>
              <p className="register-success-line">
                <MailCheck aria-hidden="true" size={14} /> OUR TEAM WILL VERIFY YOUR REGISTRATION AND CONTACT YOU SOON THROUGH THE TEAM LEADER'S GMAIL ADDRESS.
              </p>
              <p className="register-demo-note">
                Please keep your payment screenshot and registration ID safe.
              </p>
              <button type="button" className="secondary-cta" onClick={reset}>[ NEW ENTRY ]</button>
            </div>
          )}
        </div>
      </div>

      <Link className="sticky-register-cta" to="/register">REGISTER NOW</Link>
    </section>
  );
}

export const registrationEventCount = catalogEvents.length;
